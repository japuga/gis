...
Friend Class frmCustFee
    ...
    Private rsFee As SqlDataReader
	...
	Private Sub load_dgFee()
        ...
        rsFee = cmd.ExecuteReader()
        If rsFee.HasRows() Then
...
ErrorHandler: 
	...
	Private Sub dgFee_DblClick(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles dgFee.DblClick
		update_fee()
	End Sub
...
ErrorHandler: 
	...
	Private Sub set_fee_record(ByRef bFlag As General.modo)
		...
		Select Case bFlag
			...
			Case General.modo.UpdateRecord
				If dgFee.Row >= 0 Then
					gCustFeeRecord.nFeeValue = CDbl(dgFee.Columns("Value").Text)
					gCustFeeRecord.sFeeDesc = dgFee.Columns("Desc").Text
					gCustFeeRecord.nFeeTypeId = CShort(dgFee.Columns("fee_type_id").Text)
					gCustFeeRecord.nFeeId = CShort(dgFee.Columns("fee_id").Text)
					gCustFeeRecord.sFeeActive = dgFee.Columns("Active").Text
	...
	Private Function delete_fee() As Boolean
		...
		On Error GoTo ErrorHandler
		If dgFee.SelBookmarks.Count = 0 Then
		...
		If MsgBox("Do you want to delete this record.", MsgBoxStyle.YesNo + MsgBoxStyle.Question, "GLM Message") = MsgBoxResult.Yes Then
			...
			sStmt = "SELECT fee_id FROM FeeService " & " WHERE fee_id =" & Str(CDbl(dgFee.Columns("fee_id").Text))
            cmd.CommandText = sStmt
            rsLocal = cmd.ExecuteReader()
			
			sStmt = "DELETE FROM FeeService " & " WHERE fee_id =" & Str(CDbl(dgFee.Columns("fee_id").Text))
			...
			'Header
			sStmt = "DELETE FROM Fee " & "WHERE fee_id =" & Str(CDbl(dgFee.Columns("fee_id").Text))
            cmd.CommandText = sStmt
